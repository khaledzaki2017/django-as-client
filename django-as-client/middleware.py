...
class AnnounceCookieMiddleware(object):
    """
    call the  API  and get a token for the currently logged in user and set the token received from the API as a cookie. 
    """
    def get_token(self, request):
        
        #get a token for logged in user
    ...
    def set_announce_cookie(self, request, response):
        ...
        expires = datetime.datetime.strftime(
            date + cookie_age, cookie_date_str
        )
    ...
    def process_response(self, request, response):
        ...
        if not request.user.is_authenticated() and announce_cookie_name in request.COOKIES:
            # If there is no authenticated user attached to this request,
            # but there is a token cookie still present, delete it.
            # on logout.
